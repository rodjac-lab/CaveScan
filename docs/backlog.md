...
2. [x] **Routeur SQL pour questions factuelles — livré puis remplacé** (commits 474747d + 5953bfc + aa2964c, 2026-04-22/23, doc archive `docs/celestin-sql-routing-refactor.md`). Classifier LLM dedie (`classify-celestin-intent`, Gemini Flash Lite) + 5 builders deterministes dans `sqlRetrievalRouter.ts`. La premiere version regex a ete abandonnee apres faux positifs systemiques. Le 2026-05-01, ce chemin a ete retire du runtime principal : Claude Haiku 4.5 choisit lui-même les tools factuels user-scopes dans l'appel principal. Le classifier reste legacy/debug.
...
- [x] **Alleger le prompt anti-hallucination** dans `supabase/functions/celestin/context-builder.ts` (2026-04-26). Reduction ~120 tokens sur les modes cellar_assistant/tasting_memory/wine_conversation : bloc SQL retrieval condense de 10 a 5 bullets (regles redondantes avec sqlRetrievalRouter supprimees), memories section condensee. Eval LLM 40/40 pass, latence percue en baisse au dogfood. Detail rule par rule dans `docs/celestin-prompt-audit-2026-04.md`. Chantier dépendant : nouvelle infra de tests Vitest (suite eval gated par RUN_LLM_EVAL, snapshot per cognitive mode, responseLineCount, 3 nouveaux scenarios). Prochaines etapes (audits separes) : `user-prompt.ts`, `extract-chat-insights/index.ts`, `crossSessionMemory.ts`.
