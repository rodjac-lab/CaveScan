...
4. [ ] **Durcir `extract-chat-insights`**. Les facts mal categorises polluent la compilation meme apres les filtres (threshold 0.7) : etats de cave classes `cellar_intent` au lieu d'observations ponctuelles, feedback produit classe `wine_knowledge`, preferences inferees d'une seule degustation notees avec confiance elevee malgre la regle existante (lignes 72-74). Besoin : 3 regles prompt explicites (interdire les inventaires de cave en `cellar_intent`, interdire le feedback Celestin en `wine_knowledge`, capper la confiance a 0.6 pour les preferences inferees de moins de 2 degustations).
