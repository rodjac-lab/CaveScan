...
- [ ] **Adapters tools provider-agnostic pour Gemini/OpenAI**. Aujourd'hui les tools factuels sont executes dans le chemin Claude. Si on veut autoriser Gemini/OpenAI sur des tours `force_*` ou `source_required`, il faut un contrat tool-call agnostique : schemas par provider, execution backend unique, boucle follow-up normalisee, traces et tests provider par provider. Tant que ce chantier n'est pas fait, les fallbacks non-tooles restent bloques sur faits exacts.
