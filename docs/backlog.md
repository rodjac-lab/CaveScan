...
7. [ ] **Dedupe fuzzy des facts compiles**. Le LLM extracteur produit parfois des facts quasi-identiques avec un mot en plus ("vins de Macle difficiles a trouver" + "vins de Macle difficiles a trouver actuellement"). La dedupe actuelle dans `pickTopFacts` compare les strings entieres en lowercase, donc voit deux facts distincts. Ameliorer le dedup en normalisant : retirer adverbes de fin (`actuellement`, `desormais`, `pour le moment`), comparer les premiers ~80% de la chaine, ou similarity Jaccard sur les tokens.
8. [ ] **Matching d'entite plus fin**. Aujourd'hui `extractWineEntities` dans `compiled-profile.ts` rate les entites trop longues (ex : "Aloxe-Corton Domaine Celine Perrin" ne matche pas un tasting `domaine: "Domaine Celine Perrin"` car l'entity est plus longue que le haystack). Splitter les entites en sous-tokens et tester l'inclusion dans les deux sens, ou maintenir une liste de domaines connus depuis `bottles`/`tasting_notes` pour faire un matching exact.
